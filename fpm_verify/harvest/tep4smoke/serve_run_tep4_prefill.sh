...
engine_command=(python3 -m dynamo.vllm --model /workspace/model_cache/models--MiniMaxAI--MiniMax-M2.7/snapshots/d494266a4affc0d2995ba1fa35c8481cbd84294b --served-model-name MiniMaxAI/MiniMax-M2.7 --tensor-parallel-size 4 --pipeline-parallel-size 1 --data-parallel-size 1 --enable-expert-parallel --kv-cache-dtype fp8 --distributed-executor-backend mp --distributed-timeout-seconds 1800 --no-enable-log-requests --no-async-scheduling --max-model-len -1 --max-num-batched-tokens 8192 --dump-config-to /results/resolved-config-node0.json)
