...
| b200 (**current**) | `gc-steady-b200-randtok2-20260812` | = d719cca-gc-steady + salt-paired randomized benchmark inputs. Digest `sha256:b0535deb…`. Live-validated by the 2026-08-12 GLM-NVFP4 smoke (0 errors, prefix points pass). **No 16xfix layer** — single-node only today; for a future 16-GPU B200 run, first try the x86 h200 tag `gc-steady-randtok2-20260812` (B200 nodes are x86; it carries 16xfix+randtok2) with a smoke before building anything new |
| — (experiment only, never for formal collection) | `gc-steady-randtok2-n11-20260813` | = randtok2 + tline logging + **N=11 steady-step decode measurement** (inject ctx-11, extra_steps=11, expected_fpms=12). Validation image for the deep-KV timing fix: same-boot spread at 7 deep-KV coordinates drops 104-179% → 0.2-1.1% (median over steady steps 2..11). Digest `sha256:56f707c4…`. See HANDOVER_DYNAMO_FPM §6.3(d) |
| — (experiment only, never for formal collection) | `gc-steady-randtok2-n11gpu-20260813` | = n11 + gpu_model_runner execute_model wrapper (per-step CUDA event pair + CPU stamps, deferred readback; WTL JSON lines). Delivered the GPU alibi verdict: scheduler-side anomalies at 0.34-1.45× while GPU held 0.99-1.00× — the timing artifact is entirely CPU-side. Digest `sha256:83e1564c…` |
| — (experiment only, superseded by n11) | `gc-steady-randtok2-twostep-20260813` | = randtok2 + tline + N=2 steady steps. Showed N=2 only relocates the artifact (extreme 7.6-13.8ms collapses). Digest `sha256:8d3347f6…` |
| — (experiment only, never for formal collection) | `gc-steady-randtok2-tline-20260812` | = randtok2 + logging-only timeline layer (TLINE JSON lines in engine stdout: per-update arrival/t_sched/super duration, inject/steady-step construction durations). Built 2026-08-12 to reproduce the giant-KV wall_time misattribution (HANDOVER_DYNAMO_FPM §6.1); reproduced 8/450 anomalies. Digest `sha256:d9514570…` |
