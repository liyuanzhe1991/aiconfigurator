...
| 点 | 实测 | 模型现值 | 偏差 | 机制 |
|---|---|---|---|---|
| b=600, kv=65400 | 88.21 ms | 44.37 ms | **+98.8%** | 批轴邻居选择跳过 b=513(eager 悬崖行),用 b=512(graph, 36.27)→b=1024(eager, 89.10)直线桥接 |
...
- tp4/kv65536: b=512→36.27ms(graph),**b=513→98.72ms(eager)**,b=1024→89.10ms;
  tp8 同构(26.50 / 92.82 / 91.92)。精确命中 b=513 能返回行值,
  但 b∈(513,1024) 的离网查询绕开了它——批轴 regime 边界被插值器无视。
- 修法与 §2.1 同一把刀:decode 批轴按 capture_sizes 分段,**段内插值、段间禁桥**;
  graph 段内用"向上 pad 到下一 capture size"的阶梯语义(悬崖对 (512,513)、(8,9)、
  (16,17)… 正是采集器留给你的段边界标记)。
