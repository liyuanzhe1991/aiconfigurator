...
### §5 最终设计(2026-08-12 定稿,替代早先"snap/子表 k-NN"表述):decode 批轴退役 k-NN

**观察**:decode 批轴格是满的——每个 capture 档都有行,且每档带 (x, x+1) 对,
外加 eager 端点 {513, 1024}。任何查询 b 都存在"同段两端采集行"。散点 k-NN 是给
格不满的空间(GEMM (n,k))设计的;批轴每段有括号,不需要迁移,用了必混票。

**设计**(全在 FPM op 层,共享引擎零改动):

1. 段划分(建表时数据驱动):相邻对跳变 ≥2× → graph/eager 大边界;
   capture 档序列(由对的存在性导出)→ 细段边界 (x_k, x_{k+1}];
2. 查询 query(b, kv):定段 → 对段两端行各做一次**精确站点查询**(own-site
   路径,KV 沿各自曲线求值,机制复用)→ 按 b 线性混合(段内 GEMM 形状固定,
   attention 随真实条数近线性,两点括住贴合);
3. 与 #1461 已落的粗分区关系:细分区是其升级——"子表内 k-NN"换成"段内括住";
   eager 段同样两点括住 {513, 1024}。b > 1024 沿用 frontier 语义。

数值预期:(12, 8184) tp8 从 8.98(b=8 混票)→ ~9.3(实测 10.70,余差=路由带
+ragged 溢价);(600, 65400) tp4 从 44.37 → ~97.1(实测 87.5,余差=路由带)。
prefill 与 GEMM 等真散点场不动。

**教训入档**:插值器正确性来自假设与硬件行为匹配。引擎行为分段离散(capture
档、graph/eager),正确形态 = 先按物理分段、段内用最笨的插值;平滑方法的错误
额外隐蔽(b=600 → 44.37,量级合理不报错,需真机探针+数值复现才能定位)。
