...
export FPM_RUN_ID=fpm-dc6ae7fea4470986
...
engine_command=(python3 -m dynamo.vllm --model /workspace/model_cache/models--MiniMaxAI--MiniMax-M2.7/snapshots/d494266a4affc0d2995ba1fa35c8481cbd84294b --served-model-name MiniMaxAI/MiniMax-M2.7 --tensor-parallel-size 4 --pipeline-parallel-size 1 --data-parallel-size 1 --enable-expert-parallel --kv-cache-dtype fp8 --distributed-executor-backend mp --distributed-timeout-seconds 1800 --no-enable-log-requests --no-enable-prefix-caching --max-model-len -1 --scheduler-cls dynamo.vllm.instrumented_scheduler.InstrumentedScheduler --worker-extension-cls dynamo.vllm.gc_policy.FpmGcWorkerExtension --additional-config "$FPM_PROBE_ADDCFG" --dump-config-to /results/resolved-config-node0.json)
