...
export MODEL_PATH=${MODEL_PATH:-"MiniMaxAI/MiniMax-M2.7"}
export SERVED_MODEL_NAME=${SERVED_MODEL_NAME:-"MiniMaxAI/MiniMax-M2.7"}
...
\
    --tensor-parallel-size "4" --pipeline-parallel-size "1" --data-parallel-size "1" --enable-expert-parallel --kv-cache-dtype "fp8" --max-model-len "10716" --max-num-seqs "512" --max-num-batched-tokens 10204 2>&1 | sed "s/^/[Worker $w] /" ) &
done
wait
