...
export FPM_RUN_ID=fpm-cc1e40423cf4689e
...
engine_command=(python3 -m dynamo.vllm --model /workspace/model_cache/models--MiniMaxAI--MiniMax-M2.7/snapshots/d494266a4affc0d2995ba1fa35c8481cbd84294b --served-model-name MiniMaxAI/MiniMax-M2.7 --tensor-parallel-size 8 --pipeline-parallel-size 1 --data-parallel-size 1 --enable-expert-parallel --kv-cache-dtype fp8 --distributed-executor-backend mp --distributed-timeout-seconds 1800 --no-enable-log-requests --no-async-scheduling --max-model-len -1 --scheduler-cls dynamo.vllm.instrumented_scheduler.InstrumentedScheduler --worker-extension-cls dynamo.vllm.gc_policy.FpmGcWorkerExtension --additional-config "$FPM_PROBE_ADDCFG" --max-num-batched-tokens 8192 --compilation-config '{"cudagraph_capture_sizes":[1,2,4,8,16,24,32,40,48,56,64,72,80,88,96,104,112,120,128,136,144,152,160,168,176,184,192,200,208,216,224,232,240,248,256,272,288,304,320,336,352,368,384,400,416,432,448,464,480,496,512,544,576,608,640,672,704,736,768,800,832,864,896,928,960,992,1024,1056,1088,1120,1152,1184,1216,1248,1280,1312,1344,1376,1408,1440,1472,1504,1536,1568,1600,1632,1664,1696,1728,1760,1792,1824,1856,1888,1920,1952,1984,2016,2048],"max_cudagraph_capture_size":2048}' --dump-config-to /results/resolved-config-node0.json)
