...
for rate in "${request_rates[@]}"; do
    ...
    curl http://127.0.0.1:30000/start_profile

    OUR_DIR=`pwd`/tmp/L2/$BASE_OUT_DIR/data/$rate
    mkdir -p $OUR_DIR
    ...
    python3 `pwd`/test/hisim/collect/bench_serving.py \
        ...
        --num-prompts 120 \
        --output-file $OUR_DIR/evict_l1.metrics.json

    curl http://127.0.0.1:30000/start_profile
    mv $SGL_HOOK_REQ_INFO_DIR/TP0.raw_request.jsonl $OUR_DIR/evict_l1.requests.jsonl
    mv $SGL_HOOK_REQ_INFO_DIR/TP0.schedule_batch.jsonl $OUR_DIR/evict_l1.schedule_batch.jsonl
    ...
    python3 `pwd`/test/hisim/collect/bench_serving.py \
        ...
        --gsp-prompts-per-group 1 \
        --gsp-num-groups 100 \
        --gsp-question-seed 123456 \
        --gsp-system-prompt-len 1024 \
        --gsp-question-len 1024 \
        --gsp-output-len 512 \
        --random-range-ratio 0.5 \
        --output-file $OUR_DIR/l2.metrics.json

    curl http://127.0.0.1:30000/start_profile
    mv $SGL_HOOK_REQ_INFO_DIR/TP0.raw_request.jsonl $OUR_DIR/l2.requests.jsonl
    mv $SGL_HOOK_REQ_INFO_DIR/TP0.schedule_batch.jsonl $OUR_DIR/l2.schedule_batch.jsonl
