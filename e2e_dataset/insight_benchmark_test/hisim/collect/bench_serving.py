...
def get_available_tokens(tokenizer):
    ...
    tokens = sorted(list(tokenizer.get_vocab().values()))
    return tokens
...
def sample_generated_shared_prefix_requests(
    ...
    for _ in range(num_groups):
        _system_prompt_len = np.random.randint(
            max(int(system_prompt_len * args.random_range_ratio), 1),
            system_prompt_len + 1,
        )
        system_prompt = gen_prompt(tokenizer, _system_prompt_len)
        system_prompts.append(system_prompt)

    # Generate questions
    questions = []
    # Reset the random seed to ensure the question is different
    np.random.seed(args.gsp_question_seed)
    random.seed(args.gsp_question_seed)
    for _ in range(num_groups * prompts_per_group):
        _question_len = np.random.randint(
            max(int(question_len * args.random_range_ratio), 1),
            question_len + 1,
        )
        question = gen_prompt(tokenizer, _question_len)
    ...
    # Make sure the output length is constant
    ...
    for group_idx in tqdm(range(num_groups), desc="Generating system prompt"):
        ...
        for prompt_idx in tqdm(
            ...
            _output_len = np.random.randint(
                max(int(output_len * args.random_range_ratio), 1),
                output_len + 1,
            )
...
if __name__ == "__main__":
    ...
    group.add_argument(
        ...
        type=int,
        default=None,
        help="The random seed for questions in generated-shared-prefix dataset",
    )
    group.add_argument(
        "--gsp-output-len",
        type=int,
